...
* Acknowledgements
  ...
  :UNNUMBERED: t
  :END:
  This paper was written between 2013 to 2015 whilst the author was Visiting Fellow at CASE. Some of the analyses presented build on research conducted in 2011 and 2012 as part of the Social Policy in a Cold Climate programme, funded by the Nuffield Foundation, the Joseph Rowntree Foundation and the Trust for London. The UMBR small-area poverty proxy dataset used here was also developed within this research programme.

  The paper has benefited from ongoing exchanges with colleagues at CASE over the years. Hannah Aldridge (New Policy Institute) and Antoine Paccoud (LSE) read an earlier draft of the paper and provided many valuable comments and criticisms. Abigail McKnight made numerous sharp-eyed editorial improvements. Any errors remain, of course, solely the author's responsibility.
...
#+BEGIN_SRC R :exports results :results output graphics :file fig/econstat_borough.pdf :width 4.5 :height 7.5
  ...
  ggplot(foo.o, aes(x=LA.SHORTNAME, group=variable, y=value/1000, fill=variable)) +
      ...
      scale_fill_manual("", values=colpal.qual,
                        labels=c("\"Working\ (head and/or spouse working)"\n",
